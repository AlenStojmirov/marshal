...
const ProductDetails = () => {
    ...
    const handleSellProduct = async (e) => {
        ...
        const newSoldEntry = {
            size: selectedSize, price: salePrice, soldDate: new Date().toISOString(),
        };

        const updatedSold = product.sold ? [...product.sold, newSoldEntry] : [newSoldEntry];

        const updates = {
            [`products/${productId}/sizes`]: updatedSizes, [`products/${productId}/sold`]: updatedSold,
        };

        try {
            await update(ref(database), updates);
            setProduct({
                ...product, sizes: updatedSizes, sold: updatedSold,
            });
            setSelectedSize('');
    ...
    return (firstLoad ? <div className="container my-4">
        <div className="row">
                    ...
                    <div className="card-body">
                        ...
                        <div className="available-sizes mt-4">
                            ...
                            <ul className="list-group">
                                {product.sizes?.map((sizeObj, index) => (<li
                                    key={index}
                        ...
                        {/* Sold Sizes Section */}
                        {((product?.sold?.length > 0) && (role === "admin")) && (<div className="sold-sizes mt-4">
                            <h4 className="text-center">Продадени величини</h4>
                            <ul className="list-group">
                                {product?.sold?.map((soldObj, index) => (<li key={index}
